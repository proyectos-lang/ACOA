...
import {
  crearEstampadorAction,
  editarEstampadorAction,
  toggleEstampadorActivoAction,
  eliminarEstampadorAction,
} from "@/app/(dashboard)/estampadores/actions"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function EstampadorForm({
  inicial,
  onSubmit,
  isPending,
}: {
  inicial: EstampadorRow | null
  ...
  return (
    ...
    >
      ...
      >
        {isPending ? "Guardando…" : inicial ? "Guardar cambios" : "Crear estampador"}
      </button>
    </form>
  )
}

export function EstampadoresClient({ estampadores }: { estampadores: EstampadorRow[] }) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [toast, setToast] = React.useState<{ tipo: "ok" | "error"; msg: string } | null>(null)
  const [createOpen, setCreateOpen] = React.useState(false)
  const [editRow, setEditRow] = React.useState<EstampadorRow | null>(null)
  ...
  function handleCrear(fd: FormData) {
    startTransition(async () => {
      const res = await crearEstampadorAction(fd)
      if (res.error) showToast("error", res.error)
      else {
        setCreateOpen(false)
        showToast("ok", "Estampador creado")
        router.refresh()
      }
    })
  }
  ...
  function handleEditar(fd: FormData) {
    if (!editRow) return
    startTransition(async () => {
      const res = await editarEstampadorAction(editRow.id, fd)
      if (res.error) showToast("error", res.error)
      else {
        setEditRow(null)
        showToast("ok", "Estampador actualizado")
        router.refresh()
      }
    })
  }

  function handleToggleActivo(row: EstampadorRow) {
    startTransition(async () => {
      const res = await toggleEstampadorActivoAction(row.id, !row.activo)
      if (res.error) showToast("error", res.error)
      else {
        showToast("ok", row.activo ? "Estampador desactivado" : "Estampador activado")
        router.refresh()
      }
    })
  }
  ...
  function handleEliminar() {
    if (!deleteId) return
    startTransition(async () => {
      const res = await eliminarEstampadorAction(deleteId)
      setDeleteId(null)
      if (res.error) showToast("error", res.error)
      else {
        showToast("ok", "Estampador eliminado")
        router.refresh()
      }
    })
  }
    ...
    <div className="space-y-4">
      ...
      {estampadores.length === 0 ? (
        <div className="rounded-2xl border border-stone-200 bg-white p-12 text-center">
          <p className="text-stone-400 text-sm">
            No hay estampadores registrados. Crea el primero con "Nuevo estampador".
          </p>
        </div>
      ) : (
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-lg rounded-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Nuevo estampador</DialogTitle>
          </DialogHeader>
          <EstampadorForm inicial={null} onSubmit={handleCrear} isPending={isPending} />
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={editRow !== null} onOpenChange={(o) => !o && setEditRow(null)}>
        <DialogContent className="sm:max-w-lg rounded-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Editar estampador</DialogTitle>
          </DialogHeader>
          <EstampadorForm inicial={editRow} onSubmit={handleEditar} isPending={isPending} />
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar estampador?</AlertDialogTitle>
            <AlertDialogDescription>
              Esta acción no se puede deshacer. Si el estampador tiene historial, considera
