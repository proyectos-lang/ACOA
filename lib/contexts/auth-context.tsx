...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Carga el perfil completo desde 'usuarios' (con JOIN a razon_social).
  // Si no existe una fila de perfil, igual devolvemos un usuario basico
  // a partir de la sesion de Supabase para que la plantilla funcione
  ...
  const loadProfile = React.useCallback(
    async (authUserId: string, email: string): Promise<AuthUser | null> => {
      ...
      try {
        ...
        if (error || !perfil) {
          // Sin tabla de perfil: usamos los datos de la sesion.
          return fallback
        }
        ...
        return {
          ...
          razon_social_nombre: razonSocial?.nombre_empresa ?? null,
        }
      } catch (err) {
  ...
  React.useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        // produce un DEADLOCK. Diferimos las queries con setTimeout(fn, 0).
    ...
    async (email: string, password: string): Promise<{ error: string | null }> => {
      ...
      try {
        ...
        if (!profile) {
          ...
          return { error: "Tu cuenta esta inactiva." }
        }
